...
def testing(data, key, dict_data):
    assert (
        key in data.keys()
    ), f"Проверьте название переменной {key}"
    assert isinstance(
        data[key], (float, int)
    ), f"Проверьте тип переменной {key}"
    assert round(data[key], 4) == round(dict_data[key], 4), f"Проверьте результат в переменной {key}"
...
if content:
    ...
    try:
        ...
        try:
            ...
            assert (
                "a" in loc.keys()
            ), "Проверьте название переменной a"
            assert isinstance(
                loc["a"], np.ndarray
            ), "Проверьте тип переменной a"
            assert np.array_equal(loc["a"], a_check), "Проверьте результат в переменной a"

            # b
            assert (
                "b" in loc.keys()
            ), "Проверьте название переменной b"
            assert isinstance(
                loc["b"], np.ndarray
            ), "Проверьте тип переменной b"
            assert np.array_equal(loc["b"], b_check), "Проверьте результат в переменной b"
