...
st.markdown(
    ...
    "- Получите сингулярное разложение матрицы **array_four** при помощи numpy, запишите в соответствующие переменные **u, s, vh**, где:\n"
    "   - **u** - ортогональная матрица\n"
    "   - **s** - массив, числа которого используются в диагональной матрице $\Sigma$\n"
    "   - **vh** - ортогональная матрица в уже транспонированном виде (ничего транспонировать самим не нужно)\n"
    "- Создайте переменную **result** и присвойте ей значение переменожения **u**, **s** и **vh**, где от матрицы **u** необходимо взять по 8 строк и 8 столбцов (подсказка u[:8, :8]@ ....)"
)
