...
st.markdown(
    ...
    "   - Создайте переменную **check_1** и присвойте ей результат перемножения матрицы X_matrix на нулевой вектор из **v**\n"
    "   - Создайте переменную **check_2** и присвойте ей результат перемножения нулевого вектора **v** на собственное значение **w** под нулевым индексом\n"
    "**Подсказка:**\n"
...
if content:
    ...
    try:
        ...
        try:
            # X_matrix
            assert (
                "X_matrix" in loc.keys()
            ), "Проверьте название переменной X_matrix"
            assert isinstance(
            ...
            assert np.array_equal(loc["X_matrix"], X_matrix_check), "Проверьте результат в переменной X_matrix"
