...
st.markdown(
    ...
    "   - Метод **summation** - должен принимать на вход **value** - объект типа list и **возвращать сумму элементов**. Предусмотреть исключения **TRY-EXCEPT**, **при ошибке не должно НИЧЕГО возвращатся**\n"
    "   - Метод **average** - должен находить среднее значение из списка **value_lst**, который вы унаследовали из **Number**\n"
    "   - Чтобы найти среднее значение используйте также метод **summation**. Предусмотреть исключения **TRY-EXCEPT**, **при ошибке не должно НИЧЕГО возвращатся**я\n"
