...
if content:
    ...
    try:
        ...
        assert "dept_emp" in content.lower(), "Проверьте, что вы используете таблицу dept_emp для поиска средней зарплаты в департаментах"
        assert (
            df.shape[0] == df_check.shape[0]
        ), "Проверьте размер таблицы, получаемый в ходе выполнения скрипта"
        assert "group by" in content.lower(), "Проверьте скрипт, возможно не хватает оператора группировки"
        assert df_check.equals(df), "Проверьте, что скрипт написан согласно заданию, а также, что вы не округляли значения"
