...
if content:
    ...
    try:
        ...
        try:
            ...
            x = loc["x"]
            assert x == x_check, "Проверьте значение в переменной x"
            assert 'y' in loc.keys(), "Проверьте название переменной y"
            y = loc["y"]
            assert y == y_check, "Проверьте значение в переменной y"
            assert 'result' in loc.keys(), "Проверьте название переменной result"
            result = loc["result"]
            assert result == result_check, "Проверьте значение в переменной result"
