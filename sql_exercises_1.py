...
st.markdown("- Выведите первые 10 строк из таблицы **dept_emp**\n"
            "**Примечание**: можно выводить таблицу только до 80 строк при тестировании скрипта\n"
            "Пример скрипта:")
...
if content:
    ...
    try:
        ...
        assert len(set(df.columns)^set(df_check.columns)) == 0, "Проверьте название таблицы"
        assert df.shape[0] == df_check.shape[0], "Проверьте размер таблицы, получаемый в ходе выполнения скрипта"
